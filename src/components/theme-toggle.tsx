  ...
  return (
    <div className="relative flex h-10 w-[5.5rem] cursor-pointer items-center justify-between rounded-full bg-gradient-to-br from-gray-50 to-white px-2 shadow-md shadow-gray-400 transition-all duration-500 dark:from-gray-700 dark:to-customGray-dark dark:shadow-black">
      {mounted && (
        <>
          <BsSun
            className={`${iconCN} z-30 text-primary-light`}
          ...
          {theme === 'light' ? (
            ...
            <BsMoonFill
              className={`${iconCN} z-30 text-primary-base`}
              onClick={() => setTheme('dark')}
            />
          )}
        </>
      )}
      <span className="absolute left-[8px] h-8 w-8 rounded-full bg-primary-base transition-all duration-500 dark:left-auto dark:translate-x-[40px] dark:bg-primary-light" />
    </div>
  );
}
